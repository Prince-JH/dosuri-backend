...
class TestUser:
    ...
    def test_create_user_if_not_exist(self):
        um.User.objects.get_or_create(username='test')[0]
...
class TestUserSetting:
    @pytest.mark.django_db
    def test_create_default_setting(self, user_dummy):
        setting = um.UserSetting.objects.create_default_setting(user_dummy)

        assert setting.agree_marketing_personal_info is False
        assert setting.agree_general_push is True
        assert setting.agree_marketing_push is False
        assert setting.agree_marketing_email is False
        assert setting.agree_marketing_sms is False
